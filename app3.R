...
data_long <- Merged_Risk_Premium %>%
  ...
  distinct()  # Ensure distinct rows to avoid duplication
...
server <- function(input, output) {
  output$yearPlot <- renderPlotly({
    ...
    p <- ggplot(year_data, aes(x = Sev_, y = Freq_)) +
      geom_point(aes(label = Statutory_Class, color= Statutory_Class), size = 4, alpha = 0.8) +
      geom_label_repel(aes(label = paste("Statutory_Class:", Statutory_Class)),
